...
1. **The worker has Cerebras; the api does not.** Extraction, classification
   and explanation all run in the worker because the Cerebras credentials live
   on the worker service. The api process has only OpenRouter, so a call here
   passes an empty `CEREBRAS_BASE_URL`, falls back to OpenRouter, and 400s.
...
async def run_intelligence(request: Request, opportunity_id: uuid.UUID) -> dict:
    ...
    `start_sourcing` answers. The worker does the three Cerebras calls.
    """
