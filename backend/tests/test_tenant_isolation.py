"""Tenant/User integrity — the foundation multi-tenancy rests on (§18).

These caught a real defect: `User.tenant_id` was originally declared without a
ForeignKey, which left `Tenant.users` with no join condition (NoForeignKeysError
on first ORM use) and let the database accept orphan tenant_ids. Core-only
health checks did not exercise the mapper, so nothing surfaced it.
"""
...
async def test_mappers_configure() -> None:
    """Fails loudly if a relationship loses its join condition."""
    configure_mappers()


async def test_tenant_user_relationship_round_trips() -> None:
    async with SessionLocal() as s:
        t = _tenant()
        s.add(t)
        await s.flush()
        s.add(User(tenant_id=t.id, email="recruiter@acme.sg", display_name="R"))
        await s.commit()
        tenant_id = t.id
...
async def test_user_cannot_reference_a_nonexistent_tenant() -> None:
    """The FK is what stops Agency A's data being written under a bogus tenant."""
    async with SessionLocal() as s:
        s.add(User(tenant_id=uuid.uuid4(), email="ghost@nowhere.sg"))
        with pytest.raises(IntegrityError):
            await s.commit()


async def test_deleting_a_tenant_removes_its_users() -> None:
    """Supports the §30 tenant deletion workflow — no rows left behind."""
    async with SessionLocal() as s:
        t = _tenant()
        s.add(t)
        await s.flush()
        s.add(User(tenant_id=t.id, email="gone@acme.sg"))
        await s.commit()
        tenant_id = t.id
