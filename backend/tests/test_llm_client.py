...
# --- complete_json_anthropic (GLM / Z.AI Anthropic Messages API) ---

# The Anthropic-compatible provider for the Job Intelligence pipeline. The
# wire format is `content[].text` instead of `choices[0].message.content`,
# `x-api-key` instead of `Authorization: Bearer`, and coding-tool headers.
# The mock transport intercepts the request, so the endpoint is never dialled.

_ANTHROPIC_FIXTURES = {
    "JOB_INTELLIGENCE_LLM_ENABLED": True,
    "JOB_INTELLIGENCE_LLM_API_KEY": "test-glm-key",
    "JOB_INTELLIGENCE_LLM_BASE_URL": "https://glm.test/v1",
    "JOB_INTELLIGENCE_LLM_MODEL_NAME": "GLM-5.3",
    "JOB_INTELLIGENCE_LLM_MAX_TOKENS": 65536,
    "JOB_INTELLIGENCE_LLM_TIMEOUT_S": 60,
    "JOB_INTELLIGENCE_LLM_SESSION_NAME": "expressautomate-test",
}


@pytest.fixture(autouse=True)
def _glm_settings(monkeypatch):
    for key, value in _ANTHROPIC_FIXTURES.items():
        monkeypatch.setattr(settings, key, value)


def _anthropic_transport(payload, status=200):
    """Build a mock transport that returns an Anthropic-format response."""
...
async def test_anthropic_returns_parsed_json_and_usage():
    """Happy path: the model returns a valid JSON object with usage stats."""
    payload = {
        "content": [{"type": "text", "text": '{"role": "Logistics Manager"}'}],
        "usage": {"input_tokens": 100, "output_tokens": 20},
        "model": "GLM-5.3",
    }

    result = await complete_json_anthropic(
        "prompt", model="GLM-5.3", transport=_anthropic_transport(payload)
    )

    assert result.data == {"role": "Logistics Manager"}
    assert result.prompt_tokens == 100
    assert result.completion_tokens == 20
    assert result.latency_ms >= 0
...
async def test_anthropic_sends_the_coding_tool_headers():
    """The x-api-key, x-session-id and x-claude-code-session-id headers must
    be present so Z.AI's coding-plan quota recognises the traffic."""
    ...
    await complete_json_anthropic(
        "prompt", model="GLM-5.3", transport=httpx.MockTransport(capture)
    )

    headers = captured["headers"]
    assert headers.get("x-api-key") == "test-glm-key"
    assert headers.get("x-session-id") is not None
    assert headers.get("x-claude-code-session-id") == headers.get("x-session-id")
    assert headers.get("x-session-name") == "expressautomate-test"
    assert headers.get("anthropic-version") == "2023-06-01"
    # The body is Anthropic Messages format, not OpenAI chat/completions
...
async def test_anthropic_non_json_content_raises_llm_invalid_json():
    """Prose instead of JSON is caught by the same parser as the OpenAI path."""
    payload = {
        "content": [{"type": "text", "text": "Sure! Here are the jobs:"}],
        "usage": {},
        "model": "GLM-5.3",
    }
...
async def test_anthropic_json_wrapped_in_code_fence_is_recovered():
    """Same fence-stripping as the OpenAI path — models do this constantly."""
    payload = {
...
async def test_anthropic_empty_content_raises_llm_no_content():
    """A reasoning model that spent its whole budget returns no text block."""
    payload = {
        "content": [
            {
                "type": "reasoning",
...
async def test_anthropic_retries_transport_errors():
    """Same 3-attempt retry loop as the OpenAI path."""
    calls = {"n": 0}
...
async def test_anthropic_tls_hang_surfaces_as_retryable_timeout():
    """Same CancelledError → TimeoutError handling as the OpenAI path."""
    calls = {"n": 0}
